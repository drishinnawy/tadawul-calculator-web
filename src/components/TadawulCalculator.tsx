...
export default function TadawulCalculator() {
  ...
  const formatNumber = (num: number | string) => (isNaN(Number(num)) ? "-" : Number(num).toLocaleString("ar-SA", { maximumFractionDigits: 2 }));
  ...
  const handlePurchaseChange = (id: number, field: string, value: string) => setPurchases(purchases.map((p) => (p.id === id ? { ...p, [field]: value } : p)));
  ...
  const handleClearAll = () => {
    setAmount(""); setPrice(""); setShares(""); setPurchases([{ id: 1, shares: "", price: "" }]);
    setSellShares(""); setSellPrice(""); setNetProceeds(0); setProfitOrLoss(0);
  ...
  return (
    <div className={`p-6 rounded-2xl ${themeClasses}`}>
      ...
      <Card className="mb-6 bg-white/80 shadow-md border border-blue-200">
        <CardHeader><CardTitle className="flex items-center gap-2 text-blue-700"><Info /> إعدادات عامة</CardTitle><CardDescription>إعدادات تؤثر على دقة الحسابات</CardDescription></CardHeader>
      ...
      <Card className="mt-6 bg-white/80 shadow-md border border-amber-100">
        ...
        <CardContent>
          ...
          <Input type="number" placeholder="أدخل نسبة البيع %" className="mt-3" onChange={(e) => {
            ...
            if (!isNaN(val)) setSellShares(Math.floor((totalShares * val) / 100).toString());
          }} />
          ...
          <Card className="mt-6 bg-white/80 shadow-md border border-blue-100">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between"><span>متوسط التكلفة الجديد:</span><span className="
