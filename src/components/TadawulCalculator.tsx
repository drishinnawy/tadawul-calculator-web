...
export default function TadawulCalculator() {
  ...
  // -----------------------------
  // 🟦 البيع الجزئي — يجب أن تأتي قبل profitPerShare
  // -----------------------------
  const [sellShares, setSellShares] = useState("");
  const [sellPrice, setSellPrice] = useState("");

  // ⚠️ averagePrice هنا هو متوسط التكلفة العام — وليس useState
  // لذلك لا يجب تعريفه بـ useState مرة أخرى
  const [totalShares, setTotalShares] = useState(0);

  // -----------------------------
  // 🟦 حساب الربح لكل سهم — المكان الصحيح 100%
  // -----------------------------
      ...
      <div className="space-y-3">
        ...
        <Input
          ...
          onChange={(e) => {
            ...
            if (!isNaN(v))
              setSellShares(Math.floor((totalSharesCalc * v) / 100).toString());
          }}
        />
