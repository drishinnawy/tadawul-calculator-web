...
export default function TadawulCalculator() {
  ...
  const totalCost = purchases.reduce((sum, p) => sum + Number(p.shares || 0) * Number(p.price || 0), 0);
  ...
  return (
    <div className={`p-6 rounded-2xl ${themeClasses}`}>
      ...
      <Card className="mb-6 bg-white/80 shadow-md border border-emerald-100">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-emerald-700"><Calculator /> حاسبة الصفقة الرئيسية</CardTitle>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Card className="bg-gradient-to-br from-sky-50 to-white shadow-sm border">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-sky-700"><BookOpen /> أدوات السهم</CardTitle>
      ...
      <Button variant="outline" asChild className="w-full">
        ...
        <Card className="lg:col-span-2 bg-white/80 shadow-md border border-emerald-100">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-emerald-700"><Repeat /> حاسبة متوسط التكلفة</CardTitle>
...
<Card className="shadow-lg mt-8 bg-white/80 border border-amber-200">
  <CardHeader>
    ...
    <CardDescription>نظرة شاملة على محفظتك بعد الصفقات والبيع</CardDescription>
  </CardHeader>
